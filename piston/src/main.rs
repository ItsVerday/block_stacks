...
use common::{PlatformInterface, InputState};
use game::*;
...
fn event_loop(state: &mut State, game_state: &mut GameState, texture_info: &mut TextureInfo, window: &mut PistonWindow) {
...
fn do_ticks(state: &mut State, game_state: &mut GameState) {
	...
	while state.accum_time >= state.ticks_executed {
		...
		tick(game_state, &mut state.interface, 1.0 / state.tickrate as f64);
...
fn do_draw(event: &Event, state: &mut State, texture_info: &mut TextureInfo, window: &mut PistonWindow) {
	for x in 0..state.interface.width {
		for y in 0..state.interface.height {
			...
			texture_info.buffer.put_pixel(x as u32, y as u32, image::Rgba(state.interface.palette[palette_index]));
		}
	}

	texture_info.texture.update(&mut texture_info.context, &texture_info.buffer).unwrap();
	window.draw_2d(event, |context, graphics, device| {
		...
		if width_scale < height_scale {
			transform = transform.trans(0.0, (height - state.size.1 as f64 * width_scale) / 2.0).scale(width_scale, width_scale);
		} else {
			transform = transform.trans((width - state.size.0 as f64 * height_scale) / 2.0, 0.0).scale(height_scale, height_scale);
		}
...
fn create_texture_info(window: &mut PistonWindow, state: &State) -> TextureInfo {
	...
	let texture: G2dTexture = Texture::from_image(&mut context, &buffer, &TextureSettings::new().filter(Filter::Nearest)).unwrap();
