use std::{collections::HashMap, io::{Cursor, Read}, path::Path, fs::File};

use rodio::{source::Buffered, Decoder, Sink, OutputStream, OutputStreamHandle, Source};
...
impl Audio {
    ...
    pub fn add<S: AsRef<str>, P: AsRef<Path>>(&mut self, name: S, path: P) {
        ...
        self.add_data(name, file_vec);
    }

    pub fn add_data<S: AsRef<str>>(&mut self, name: S, file_vec: Vec<u8>) {
    ...
    pub fn play<S: AsRef<str>>(&mut self, name: S) {
        ...
        let buffer = match self.clips.get(name.as_ref()) {
            Some(clip) => clip,
            None => return
        }.clone();
